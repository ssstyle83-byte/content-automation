...
class AuctionCrawler(BaseCrawler):
    ...
    STORE_URL = "https://stores.auction.co.kr/meditial"
    ...
    def _get_product_urls(self) -> list:
        ...
        while True:
            ...
            # JS a.href property 기반 추출 (옥션 상품 URL 패턴)
            urls = self.page.evaluate("""
                () => {
                    ...
                    Array.from(document.querySelectorAll('a')).forEach(a => {
                        const href = a.href || '';
                        // 옥션 상품 URL: ItemNo= 또는 /Item?... 또는 itempage3.auction.co.kr
                        if ((href.includes('ItemNo=') || href.includes('auction.co.kr/Item') ||
                             href.includes('itempage') && href.includes('auction')) && !seen.has(href)) {
                            seen.add(href);
                            urls.push(href);
                        }
                    });
